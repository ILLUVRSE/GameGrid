...
export const renderMatchScreen = (options) => {
  ...
  controls.innerHTML = `
    <div><strong>Move</strong> Drag / Mouse</div>
    <div class="touch-only"><strong>Touch</strong> Drag paddle or use stick</div>
  `;

  const touchControls = document.createElement("div");
  touchControls.className = "match-touch";
  touchControls.innerHTML = `
    <div class="touch-stick" aria-label="Movement stick">
      <div class="stick-base"></div>
      <div class="stick-handle"></div>
      <div class="stick-label">Drag to move</div>
    </div>
    <div class="touch-guide">
      <span>Thumbstick control for quick moves.</span>
      <span>Lift to stop.</span>
    </div>
  `;
  ...
  layout.appendChild(touchControls);
  ...
  const playPaddleSound = (hitSpeed) => {
    if (!audioCtx) return;
    const now = audioCtx.currentTime;
  ...
  const applyDirectionalInput = (vector, dt) => {
    if (Math.abs(vector.x) + Math.abs(vector.y) < 0.01) return;
    const speed = playerSpeed * 0.92;
    inputTarget.x = clamp(inputTarget.x + vector.x * speed * dt, player.r, centerX - player.r);
    inputTarget.y = clamp(inputTarget.y + vector.y * speed * dt, player.r, HEIGHT - player.r);
  };

  const touchStick = touchControls.querySelector(".touch-stick");
  const handleStickMove = (event) => {
    if (!touchStick || event.pointerId !== joystickPointerId) return;
    event.preventDefault();
    const rect = touchStick.getBoundingClientRect();
    const centerX = rect.left + rect.width / 2;
    const centerY = rect.top + rect.height / 2;
    const maxRadius = Math.max(40, rect.width / 2 - 26);
    ...
    joystickVector = { x: offsetX / maxRadius, y: offsetY / maxRadius };
    touchStick.style.setProperty("--stick-x", `${offsetX}px`);
    touchStick.style.setProperty("--stick-y", `${offsetY}px`);
  };
  ...
  const handleStickDown = (event) => {
    if (!touchStick || joystickPointerId !== null) return;
    event.preventDefault();
    joystickActive = true;
  ...
  touchStick?.addEventListener("pointerdown", handleStickDown, { passive: false });
  touchStick?.addEventListener("pointermove", handleStickMove, { passive: false });
  touchStick?.addEventListener("pointerup", handleStickUp, { passive: false });
  touchStick?.addEventListener("pointercancel", handleStickUp, { passive: false });
  touchStick?.addEventListener("pointerout", handleStickUp, { passive: false });
  ...
  const update = (now) => {
    ...
    while (remaining > 0 && !scored) {
      ...
      if (joystickActive) {
        lastPointerType = "touch";
        applyDirectionalInput(joystickVector, slice);
      }
  ...
  return {
    ...
    cleanup: () => {
      ...
      touchStick?.removeEventListener("pointerdown", handleStickDown);
      touchStick?.removeEventListener("pointermove", handleStickMove);
      touchStick?.removeEventListener("pointerup", handleStickUp);
      touchStick?.removeEventListener("pointercancel", handleStickUp);
      touchStick?.removeEventListener("pointerout", handleStickUp);
