...
const normalizeStat = (value: number) => clamp((value - 50) / 50, 0, 1);

const lerp = (min: number, max: number, t: number) => min + (max - min) * t;

const buildSkaterModifiers = (attributes: Record<string, number>) => {
...
const buildGoalieModifiers = (attributes: Record<string, number>) => {
